...
in calculating the R\'enyi entanglement entropies). Finally, although the 
...
$N_{\textrm{rep}}$ identical copies of the system are simulated. Each copy is at a different temperature 
in the range $T_{\textrm{max}}-T_{\textrm{min}}$. The standard Metropolis sweeps at 
each temperature are supplemented with parallel tempering moves, which allow 
to exchange the spin configurations of replicas with neighboring temperatures. 
In our simulations we choose $T_{\textrm{min}}=...$ and $T_{\textrm{max}}=...$. 
Each Monte Carlo sweep consists of $...$ Metropolis steps and $...$ parallel 
tempering moves. For each disorder realization we perform $...$ sweeps for $L=...$. 
The disorder average is performed over $...$ different disorder realizations.  
