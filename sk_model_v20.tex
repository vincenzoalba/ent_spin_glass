...
Here we discuss the R\'enyi entropies and the mutual information in the 
infinite-range Ising model withoud disorder defined by the hamiltonian 
%
\begin{equation}
\label{is-clean}
...
\end{equation}
%
Here we consider the situation without external magnetic field, i.e., 
$h=0$ and fix $J=1$. 

The calculation of the booklet partition function $Z(\omega,n,\beta)$ is 
done using the same techniques as in section~\ref{}. The result reads 
%
\begin{multline}
\label{clean-is-Z}
Z(\omega,n,\beta)=
...
\end{multline}
%
Using the saddle point approximation one obtains 
%
\begin{multline}
\label{clean-is-F}
\log Z(\omega,n,\beta)\approx
...
\end{equation}
%
For $n=2$, Eq.~\eqref{clean-is-spe} become 
%
\begin{align}
...
\end{align}
%
The system has the solution $z_1=z_2=0$ for $\beta\le\beta_c=1/(1+\omega)$, 
and $z_1=z_2\ne 0$ for $\beta>\beta_c$. 
