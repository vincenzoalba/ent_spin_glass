...
paramagnetic behavior, consistent with the regular S-K model. The temperature 
of the paramagnetic-glassy transition depends 
...
of the Parisi solution, we provide analytical results for $S_n$, ${\mathcal I}_n$, 
...
corresponding densities $S_n/N$ and ${\mathcal I}_n/N$, in the thermodynamic limit. 
...
\emph{replica trick}~\cite{cardy-book}, not to be confused with the replica trick used in calculating the R\'enyi entanglement entropies). Finally, although the applicability of the S-K 
...
definition) using another \emph{replica trick}\cite{nakagawa-2009,buividovich-2008}. This consists of $n$ independent and identical replicas (the 
...
\end{equation}
%
Notice that Eq.~\eqref{renyi-intro} can also be used for quantum systems 
by replacing the sum over the probabilities of each state in region $A$ with the trace over the 
reduced density matrix for region $A$.
...
$Z(\beta,\{J\})$ the partition function of the S-K model on the plane (i.e. the original S-K model). On the other hand, 
...
physical replicas, as in Eq.~\eqref{book-pf}. Again, both the physical and fictitious 
...
that $q_0\to 1$ and $q_0'\to 1$, for any $\omega$. Moreover, expansion 
yields  
%
\begin{align}
\label{low-t}
...
(unlike in local spin models). With this in mind we assume that we will find
volume-law behavior $[S_2]\propto N$ for all temperatures, and consider the entropy per spin 
...
the ratio trick one splits the subsystem $A$ in a set of subsystems $A_i$ such that $A_i\subset A_{i+1}$ and the largest subsystem in the set is simply $A$ itself. Then one writes 
%
\begin{align}
\label{ratio-trick}
...
four spins in $A_{i+1}\setminus A_i$ for each of the $n$ sheets, and $E_i$ is the energy associated with 
the spin configuration $i$ of a single sheet.
...
there are only $2^4$ configurations in total and the energy $E_j$ is that of all $n$ sheets together.
...
\bibitem{nakagawa-2009}
Y.~Nakagawa, A.~Nakamura, S.~Motoki, and V.~Zakharov (2009), arXiv:0911.2596

\bibitem{buividovich-2008}
P.~Buividovich and M.~Polikarpov, Nuc.\ Phys.\ B {\bf 802}, 458 (2008)
