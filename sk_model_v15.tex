...
%Clearly, $p_{i_A}\equiv\sum_{j_b\in{\mathcal C}_B}p_{i_A,j_B}$, with $p_{i_A,j_B}$ 
%being the probability that the system is in the configuration $i_A\in{\mathcal C}_A$
%in part $A$, and $j_B\in{\mathcal C}_B$ in part $B$. For a spin model at inverse 
%temperature $\beta$, one has $p_{i_A,j_B}\equiv e^{-\beta E(i_A,j_B)}/Z(\beta)$, 
%where $E(i_A,j_B)$ is the energy of the system, and $Z(\beta)$ its partition function. 
...
%Notice that $[F(\omega,n,\beta)]$ is the 
