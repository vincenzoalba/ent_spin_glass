...
First, using classical Monte Carlo simulations and analytical tools we investigate the 
...
of the model, which are introduced to perform disorder averages (\emph{replica 
...
The booklet geometry is illustrated in Fig.~\ref{cartoon}. 
Each sheet is divided into two parts $A$ and $B$ (cf. Fig.~\ref{cartoon} (a)). Then $n$ copies 
...
plotting $U/N$ as a function of $\beta$ and $\omega=0,1/4,1/2,3/4,1$. The plus symbols 
...
which is obtained from Eq.~\eqref{renyi}  Eq.~\eqref{RSB-1-logZ}. Remarkably, 
...
exhibits a vanishing behavior. Finite size effects are ``small''. Using Eq.~\eqref{logZ-ht} 
...
where it exhibits a maximum. The height of this maximum decreases as a function of $\omega$. 
Its position is not simply related to the paramagnetic-glassy transition in the standard S-K 
model. This is not surprising as $[{\mathcal I}_n]$ is constructed by combining the partition 
functions on booklets with different aspect ratios $\omega$, $1-\omega$, $\omega=0$, and 
$\omega=1$ (cf. Eq.~\eqref{MI}), and the critical temperature of the paramagnetic-glassy 
transition is a non trivial function of $\omega$ (see Fig.~\ref{beta_c}). Furthermore, the 
...
There are a only few approaches to numerically calculate the the ratio of partition functions in 
...
in the left hand side of~\eqref{ratio-trick} can be sampled efficiently in Monte 
Carlo~\cite{stephan-2014}. Notice that a similar method has also been used in 
...
in $A_{i+1}-A_i$. 
%We also gain some speed by calculating the weights in each replica seperately, 
%and then recombining the results in a smart way, meaning that we only have to explicitly 
%loop over $2 \cdot 2^4 = 32$ configurations.
...
%where $A$ changes only slightly, and build up the full quantity by taking the product of many such measurements.
...
%\bibitem{bouchaud-2003}

%J.-P.~Bouchaud, F.~Krzakala, and O.~C.~Martin, Phys.\ Rev.\ B {\bf 68}, 

%224404 (2003).

%\bibitem{boettcher-2005}

%S.~Boettcher: Eur.\ Phys.\ J.\ {\bf B46}, 501 (2005).
