...
paramagnetic behavior. The temperature of the paramagnetic-glassy transition depends 
non-trivially on the geometry of the booklet. At high-temperatures we provide the exact 
solution of the model exploiting the replica symmetry. This is the permutation symmetry 
...
\end{equation}
%
Notice that Eq.~\eqref{renyi-intro} can also be used for quantum systems by interpreting $p_{i_A}$ as 
the probability of finding part $A$ in the \emph{quantum} configuration $i_A$. For $n=1$ 
...
data for $\beta\lesssim 3$, confirming what is observed for the internal energy. 
...
it exhibits a maximum. The height of this maximum decreases as a function of $\omega$. 
...
the ratio trick one splits the subsystem $A$ as $A=\sum_{i}A_i$, where $A_i$ is a sequence of subsystems 
such that $A_i\subset A_{i+1}$. Then one writes 
%
\begin{align}
\label{ratio-trick}
...
four spins in $A_{i+1}\setminus A_i$, and $E_i$ is the energy associated with 
...
\end{align}
%
As in step $2$, $j$ runs over all the $n2^4$ configurations of the four spins living 
in $A_{i+1}\setminus A_i$, but the spins living on different sheets are identified.
